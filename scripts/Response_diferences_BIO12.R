...
temp_global <- raster('C:/Niche_overlap/Data/global_bioclimate/CHELSA_bio10_12.tif')
...
colnames(df_modelo_global) <- c('x','y','Suit','BIO12')
...
temp_regional <- raster('C:/Niche_overlap/Data/regional_bioclimate/CHELSA_bio10_12.tif')
...
colnames(df_modelo_regional) <- c('x','y','Suit','BIO12')
...
df_modelo_global_filtrado <- dplyr::filter(df_modelo_global, BIO12 >= rango[1] & BIO12 <= rango[2])
...
###plot final###
ggplot(df_final, aes(x = BIO12, y = Suit, colour = modelo))+
  geom_smooth()+
...
range(df_modelo_global$BIO12)
###grdi##
gam_global <- gam(Suit ~ s(BIO12, k = 10), data = df_modelo_global_filtrado)
...
gam_regional <- gam(Suit ~ s(BIO12, k = 10), data = df_modelo_regional)
...
bio_seq <- data.frame(
  BIO12 = seq(rango[1], rango[2], length.out = 1000)
)
...
cat("GRDI (Gradient Response Deviation Index) para BIO12:", round(grdi, 4), "\n") ##3612.94