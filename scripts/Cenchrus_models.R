#####modelos anidados para Cenchrus setaceum###
###anidamiento###
###modelo climático ipsl, concentration pathways ssp370 ssp585###
...
especie <- 'Cenchrus_setaseus'

sp_global <- read.csv('Cenchrus_global.csv')

sp_global <- sp_global[,c(3,2)]
colnames(sp_global) <- c('x','y')


sp_local <- read.csv('C:/Niche_overlap/Data/regional_bioclimate/Cenchrus_canarias.csv', sep = ';', dec = ',')
...
###new escenario##
##Additionally, regional-scale SpatRaster or a list of SpatRaster objects corresponding to the covariates 
##used to project the models at one or several different scenarios (i.e., new scenarios) can be provided.
...
setwd('C:/Niche_overlap/Modelling_results/Cenchrus-results')
...
nsdm_multiply <- NSDM.Multiply(nsdm_global,
                               nsdm_regional,
                               method = "Arithmetic", # Method for average model outputs: "Arithmetic" or "Geometric" mean
                               rescale = FALSE,
                               save.output=TRUE)
